...
def return_scraping_data():
    ...
    sql = "SELECT date_, place_name, race_number, start_time ,two_3month_2win, three_3month_2win, four_3month_2win " \
          "FROM all_race_data " \
    ...
    for i in range(0, len(buy_race_data)):
        date = buy_race_data[i][0]
        place_name = buy_race_data[i][1]
        race_number = buy_race_data[i][2]
        start_time = buy_race_data[i][3]
        ticket_type = "2連単"
        first＿arrival = 1

        second＿arrival = 2
        if buy_race_data[i][4] < buy_race_data[i][5]:
            second＿arrival = 3
        elif buy_race_data[i][5] < buy_race_data[i][6]:
            second＿arrival = 4
        else:
            pass
